(() => {
  ...
  if (reduceMotion) {
    ...
    // The CSS unpins the carousel into three visible cards here, so the two
    // that were hidden from assistive tech have to be handed back.
    trackPanels.forEach((el) => el.removeAttribute('aria-hidden'));
    return;
  }
